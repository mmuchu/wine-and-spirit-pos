...
export default function POSPage() {
  ...
  useEffect(() => {
    const onLine = () => { setIsOnline(true); processOfflineQueue(); };
  ...
  const fetchProducts = async (orgId: string) => {
    const { data, error } = await supabase.from("products").select("id, name, price, stock, sku, is_active, cost_price").eq("organization_id", orgId).eq("is_active", true).order("name");
  ...
  const fetchSettings = async (orgId: string) => {
    ...
    if (data) { setVatEnabled(data.vat_enabled ?? true); setShopName(data.shop_name || "KENYAN SPIRIT"); setShopAddress(data.address || "Nairobi, Kenya"); }
  };
  ...
  const processOfflineQueue = async () => { if (!organizationId) return; const res = await offlineService.syncQueue(supabase, organizationId); updatePendingCount(); if (res.synced > 0) fetchProducts(organizationId); };
  ...
  const handleCompleteSale = async () => {
    ...
    if (!organizationId) { alert("Config Error."); return; }
    setProcessing(true);
    try {
      ...
      if (isOnline) {
        ...
        savedSale = offlineService.queueSale(payload);
      ...
      setLastSale({ ...savedSale, date: new Date().toISOString(), shop_name: shopName, address: shopAddress });
      setIsReceiptModalOpen(true);
      setCart([]);
      setCashReceived("");
      updatePendingCount();
    } catch (err: any) { alert(`Error: ${err.message}`); } finally { setProcessing(false); }
  };
  ...
  if (!orgLoading && !isLicenseValid) {
    return (
      <div className="h-screen flex flex-col items-center justify-center bg-red-50 p-8 text-center">
        ...
        <p className="text-gray-600 mt-2 mb-6">Please contact admin to renew.</p>
      </div>
    );
  }
  ...
  if (!currentShift) return (
    <div className="h-screen flex flex-col items-center justify-center bg-gray-100 space-y-4">
      ...
      <p className="text-gray-500">Start a shift to sell.</p>
    </div>
  );
    ...
    <div className="flex h-screen bg-gray-100 font-sans text-gray-800">
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative pt-8">
        ...
        <div className="p-4 bg-gray-50 border-b flex gap-2 shrink-0">
          <input type="text" placeholder="Search or scan..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="flex-1 p-4 bg-white border rounded-xl" />
          <button onClick={() => setIsScannerOpen(true)} className="p-4 bg-black text-white rounded-xl hover:bg-gray-800 shrink-0">Scan</button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto bg-gray-50 p-6 grid grid-cols-2 gap-4 content-start">
          ...
          {(searchTerm ? filteredProducts : products).map(product => (
            <button key={product.id} onClick={() => addToCart(product)} disabled={product.stock <= 0} className={`bg-white rounded-xl border p-4 text-left h-36 flex flex-col justify-between ${product.stock <= 0 ? 'opacity-50' : 'hover:border-black'}`}>
      ...
      <div className="w-[400px] bg-white border-l flex flex-col shadow-xl shrink-0">
        <div className="p-4 border-b shrink-0"><h2 className="font-bold text-lg">Current Order ({cart.length})</h2></div>
        ...
        <div className="p-4 border-t space-y-3 bg-white shrink-0">
          ...
          {paymentMethod === 'cash' && <input type="number" placeholder="Cash Received" value={cashReceived} onChange={e => setCashReceived(e.target.value)} className="w-full p-2 border rounded text-right font-bold" />}
