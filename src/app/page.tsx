...
export default function DashboardPage() {
  ...
  useEffect(() => {
    // FIX: Don't run if loading OR if ID is missing
    if (orgLoading) return;
    if (!organizationId) {
      setLoading(false); // Stop loading if no org found
      return;
    }
  ...
  // FIX: Show specific message if context is done but no ID
  if (!orgLoading && !organizationId) {
    return (
      <div className="p-8 text-center space-y-4">
        <h2 className="text-xl font-bold text-red-600">Configuration Error</h2>
        <p className="text-gray-500">Organization context missing. Please contact support or check your user setup.</p>
      </div>
    );
  }

  if (loading || orgLoading) return <div className="p-8 text-gray-500">Loading Dashboard...</div>;
