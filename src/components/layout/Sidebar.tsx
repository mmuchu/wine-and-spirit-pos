...
// Simplified Navigation - No complex types
const navigation = [
  { name: "Dashboard", href: "/", icon: HomeIcon, adminOnly: false },
  { name: "POS", href: "/pos", icon: CartIcon, adminOnly: false },
  { name: "Inventory", href: "/inventory", icon: BoxIcon, adminOnly: false },
...
export function Sidebar() {
  ...
  const visibleNavigation = navigation.filter(item => {
    if (isOwner || isAdmin) return true; // Admin sees all
    if (item.adminOnly) return false; // Hide admin items for others
    return true;
  });
  ...
  const handleCashConfirm = async (amount: number) => {
    ...
    if (shiftMode === 'open') {
      ...
      try { await shiftService.openShift(organizationId, amount); checkActiveShift(); }
